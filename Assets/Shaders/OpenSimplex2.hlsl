...
// Use this if you want to show X and Y in a plane, then use Z for time, vertical, etc.
    ...
    // Adapter function with the same name and parameters as originalSimplex
    float snoise(float3 X)
    {
        float4 result = openSimplex2_ImproveXY(X);
        return result.w;
    }

    // Adapter function with the same name and parameters as originalSimplex
    float snoise(float4 X)
    {
        float4 result = openSimplex2_ImproveXY(X.xyz);
        return result.w;
    }
